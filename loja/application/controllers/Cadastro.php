<?php defined('BASEPATH') OR exit('No direct  script  access  allowed');
	class Cadastro extends CI_Controller{
		...
		public function enviar_email_confirmacao($dados){
			...
			$this->email->from("brianmessias1@gmail.com","Confirmação de cadastro");
		...
		public function recuperar_login(){
			...
			if($this->form_validation->run() == FALSE){
				...
				$this->db->where('emai', $this->input->post('email'));
				...
				if(count($cliente == 1)){
					...
					$this->load->library('email');
					$this->email->from('brianmessias1@gmail.com', "Lojão do Terceirão");
					$this->email->to($dados['email']);
					$this->email->subject('Lojão do Terceirão - confirmação de cadastro');
					$this->email->message($mensagem);
					if($this->email->send()){
