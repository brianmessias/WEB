<?php
	...
	"<div class='row-fluid'> <dic class='span3 texto-direita" . heading("Valores",3) .
	"Produtos: " . reais($this->cart->total()) . br() . 
	"Frete: " . reais($frete) . br() .
	"Total: " . reais($this->cart->total() + $frete) . br() .
	"Pagar com cartão " . form_radio(array('name'=>'tipo_pagamento', 'value'=>'boleto')) . br() .
	"</div>" . 
	"<div class='span1'></div>" .
	"<div id='dados_cartao'>" .
	'<div class=span4>';
	$bandeiras = array('mastercard'=>'Mastercard', 'visa'=>'Visa');
	echo form_label('Bandeira do cartão de credito') .
		form_dropdown('bandeira',$bandeira) .
	...
	$parcelas = array(
		...
		2=>'2	parcela de ' . reais(($this->cart->total()+frete)/2),
		3=>'3	parcela de ' . reais(($this->cart->total()+frete)/3),
	);
	echo form_labe('Parcelamento','parcelamento') .
